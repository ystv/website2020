...
CREATE TABLE people.users (
    ...
    university_username text NOT NULL DEFAULT '',
...
COMMENT ON COLUMN people.users.username IS
'In most cases is 1 to 1 with university_username, only when they have an AD account would this be different and would likely be firstname.lastname';
COMMENT ON COLUMN people.users.university_username IS
'Their ITS Account. Doesn''t have UNIQUE constraint since some old records don''t have a username available for us to store, so we have them stored
as an empty string, which will cause collisions. This does leave us to decide later if we want to handle this case. Does make it a pickle with validation';
COMMENT ON COLUMN people.users.email IS
'Would be not null, but we have existing data';
COMMENT ON COLUMN people.users.login_type IS
'internal - uses pw in this table to authenticate. sso - uses external auth';
--
